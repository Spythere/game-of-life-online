export class Game {
  ...
  constructor() {
    this._currentGen = new Array(this._dimensions.rows) // New array with rows
      .fill(0)
      .map((row) =>
        new Array(this._dimensions.cols) // Fill array and map its contents
          .fill(0)
          .map((col) => ({
      ...
      for (let col = 0; col < this._dimensions.cols; col++) {
        ...
        for (let i = -1; i <= 1; i++) {
          for (let j = -1; j <= 1; j++) {
            ...
            if (nRow < 0 || nRow >= this._dimensions.rows) continue;

            if (nCol < 0 || nCol >= this._dimensions.cols) continue;
